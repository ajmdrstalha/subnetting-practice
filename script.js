...
function showExplanation(p) {
  ...
  expWildcard.textContent = p.wildcard;

  explainGrid.hidden = false;

  expText.innerHTML =
    `<b>How it was calculated:</b><br>` +
    `Subnet Mask comes from the CIDR: /${p.cidr} → ${p.mask}<br>` +
    `Wildcard Mask = NOT Subnet Mask → ${p.wildcard}<br>` +
    `Network = IP AND Subnet Mask → ${p.network}<br>` +
    `Broadcast = Network OR Wildcard Mask → ${p.broadcast}`;
...
answerForm.addEventListener("submit", (e) => {
  ...
  const userMask = maskInput.value.trim();
  const userBroadcast = broadcastInput.value.trim();
  const userWildcard = wildcardInput.value.trim();

  const allValid =
    isValidIp(userNetwork) &&
    isValidIp(userMask) &&
    isValidIp(userBroadcast) &&
    isValidIp(userWildcard);

  if (!allValid) {
    resultSection.className = "result error";
    resultSection.textContent = "Please enter valid IPv4 dotted-decimal addresses in all fields.";
    clearExplanation();
    return;
  }
  ...
  const okNetwork = userNetwork === currentProblem.network;
  const okMask = userMask === currentProblem.mask;
  const okBroadcast = userBroadcast === currentProblem.broadcast;
  const okWildcard = userWildcard === currentProblem.wildcard;

  const allCorrect = okNetwork && okMask && okBroadcast && okWildcard;
  ...
  if (allCorrect) {
    ...
    resultSection.className = "result error";
    resultSection.textContent =
      "Incorrect. See the correct values below.";
  }
